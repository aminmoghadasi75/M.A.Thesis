...
def return_classifier(df: pd.DataFrame, last_month: pd.DataFrame, monthly_features: list, inx: int) -> tuple:
    ...
    grid_search = GridSearchCV(rf, param_grid, cv=5, scoring=scoring, refit='f1_weighted', n_jobs=-1)
    ...
    next_month_prediction = best_rf.predict(X_last_month)
    #next_month_prediction = rf.predict(X_last_month)
...
for file in df_file_list:
    ...
    for feature_file in feature_file_list:
        ...
        if feature_file == f'RFC_{file}' :
            ...
            final_result.to_excel(f'/home/amin/thesis/src/Step2_Classification/RF/RF_{file}', index=True)
